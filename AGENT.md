...
- **Go deps**: `golang.org/x/sys`, `cobra`, `go.elara.ws/pcre`, `charmbracelet/lipgloss`, `charmbracelet/log`, `simd/archsimd` (stdlib experimental)
...
3. **No cgo, no C bindings**: Everything is pure Go. Approved deps: `golang.org/x/sys`, `cobra`, `go.elara.ws/pcre`, `charmbracelet/lipgloss`, `charmbracelet/log`, `simd/archsimd`. Do not add others without approval.
...
| `openat` | `unix.Openat` | Relative file opens from dir fd |
| `fstatat` | `unix.Fstatat` | Stat only when d_type is DT_UNKNOWN |
